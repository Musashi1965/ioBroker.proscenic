...
Existing button objects are refreshed during adapter startup so earlier
test-host installations that still have `common.read: true` are migrated to the
write-only metadata contract.
